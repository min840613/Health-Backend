...
<div class="col-md-12">
    <div class="card">
        ...
        @endif
        {!! Form::open(array('route' => 'articles.store','method'=>'POST', 'id'=>'formSubmit')) !!}
            <div class="card-body">
                    ...
                    @switch($v['type'])
                        ...
                        @case('datetime')
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10" datas="{{ old('publish') ? old('publish') : date('Y-m-d H:i')}} ">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-4">
                                    <select class="form-control" name="collaborator">
                                        <option value=""></option>
                                        <option value="e1">e1:AIWIZE</option>
                                    </select>
                                </div>
                            </div>
                            @break;
                                    ...
                                    <div class="col-sm-3">
                                        <select class="form-control" name="author">
                                            ...
                                            @foreach($AuthorsDataList as $key => $value)
                                                ...
                                                @elseif ( !old('author') && $value == auth()->user()->name )
                                                    <option value="{{$key}}" selected >{{$value}} </option>
                                                @else
...
@section('js')
    ...
    <script>
        $().ready(function(){
            ...
            @if ($aiWize)
            // aiwize
            if ($('[name="collaborator"]').length) {
                $('[name="collaborator"] option[value="e1"]').prop('selected', true);
            }
            $('#title').val('{{ $aiWize["ai_long_title"] }}');
            $('#og_title, #seo_title').val('{{ $aiWize["ai_short_title"] }}');
            var aiWizeUrl = "{{ route('aiwize.articles.index') }}/{{ $aiWize['ai_wize_id'] }}/edit";
            $.get(aiWizeUrl, function (data) {
                var editor = tinymce.get('article_content');
                setTimeout(function(){
                        editor.insertContent(data.content);
                }, 1000);
                var selectizeWize = $('#tag')[0].selectize;
                var tagArray = data.keyword.split(",");
                $.each(tagArray, function(index, value) {
                    selectizeWize.addOption({
                        text: value,
                        value: value
                    });
                    selectizeWize.addItem(value);
                });

            });
            // append hidden value to #formSubmit
            $("#formSubmit").append("<input type='hidden' name='aiWizeID' value='" + {{ $aiWize['ai_wize_id'] }} + "'>");
            @endif
