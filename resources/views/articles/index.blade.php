...
@section('content_header')
    <h1>文章總覽</h1>
    {{ Form::open(['id' => 'search', 'route' => 'articles.index', 'method' => 'get', 'style' => 'border:1px solid #ccc; padding: 5px;']) }}
    @php
        $startConfig = [
...
@section('create')
    <a href="{{route('articles.create')}}" class="btn btn-sm btn-success">新增</a>
@stop
...
@section('data_list')
    @foreach ($datas as $key => $article)
        <tr>
            ...
            <td class="align-middle" width="100px">{!!$article->articles_status == 0 ? '<i style="color: #b80000;" class="fa fa-times"></i>' : '<i style="color: green;" class="fa fa-check"></i>'!!}</td>
            <td class="align-middle" style="min-width: 160px">
                <a href="{{route('articles.show', ['article' => $article->articles_id])}}" class="btn btn-sm btn-secondary" target="_blank">預覽</a>
                @can('articles-edit')
                <a href="{{route('articles.edit', ['article' => $article->articles_id])}}" class="btn btn-sm btn-primary">編輯</a>
                @if($article->articles_status == 1)
                <button type="button" class="btn btn-sm btn-primary key_visual" onclick="key_visual('{{route('keyvisual.create', ['article_id' => $article->articles_id])}}')">
                    首頁頭條
                </button>
                @endif
                @endcan
                ...
                @if($article->articles_status == 1)
                <button type="button" class="btn btn-sm btn-primary notifications" onclick="notifications('{{route('notifications.create', ['article_id' => $article->articles_id])}}')">
                    推播
                </button>
                @endif
                @endcan
...
@section('js')
    <script>
        ...
        document.cookie = "article_query_string=" + (urlParams || "") + "; path=/";
