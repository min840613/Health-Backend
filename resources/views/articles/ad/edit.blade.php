...
<div class="col-md-12">
    <div class="card">
        ...
        @endif
        {!! Form::model($ArticleData, ['method' => 'PATCH','route' => ['articles_ad.update', $ArticleData->articles_id], 'id'=>'formSubmit']) !!}
            <div class="card-body">
                @foreach($field as $k=>$v)
                    @switch($v['type'])
                        @case('category_selectize')
                            @foreach($ArticleData->mainCategories as $key=>$value)
                            @if($value['categories_id'] != $Guangbian['categories_id'])
                        ...
                        @case('datetime')
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10" datas="{{$v['value']?$v['value']:date('Y-m-d H:i')}}">
                                ...
                                <div class="col-sm-10 row">
                                    <div class="col-sm-3">
                                        <select class="form-control" name="{{$k}}">
                                            ...
                                            <option value="{{$key}}" {{$key == $v['value'][0]?'selected':''}}>{{$value}}</option>
                                            @endforeach;
                                        </select>
                                    </div>
                                    <div class="col-sm-3">
                                        <select class="form-control" name="author_type">
                                            <option value="" >請選擇</option>
                                            <option value="1" {{$v['value'][1] == 1?'selected':''}}>報導</option>
                                            <option value="2" {{$v['value'][1] == 2?'selected':''}}>整理</option>
                                        </select>
                                    </div>
...
@section('js')
    ...
    <script>
        $().ready(function(){
            ...
            var subCategoriesArr = subCategoriesData.split(',');
            subCategoriesArr = subCategoriesArr.filter(function(val) {
                return val !== "" && val !== null && val !== undefined;
            });
            // 分類管理
