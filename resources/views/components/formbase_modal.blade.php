                    ...
                    @foreach ($editField as $key => $value)
                        ...
                        @switch($value['type'])
                            ...
                            @case('adminlte_select')
                                <x-adminlte-select2 name="{{$value['name']}}" igroup-size="md">
                            ...
                            @case('text')
                                ...
                                @if(isset($value['image_gallery']) && $value['image_gallery'] == true)
                                    <div class="form-inline">
                                        ...
                                        <div>
                                            <a id="gallery_image" style='margin-left: 10px;' class="btn btn-success btn-flat float-left px-4" href="javascript:;">圖庫</a>
                                ...
                                @elseif(!empty($value['append_button']))
                                    <x-adminlte-input type="text" name="{{$value['name']}}" placeholder="{{$value['placeholder']}}">
                                        <x-slot name="appendSlot">
                                            <x-adminlte-button theme="{{$value['append_button']['theme']}}" class="{{$value['append_button']['class_name']}}" label="{{$value['append_button']['word']}}"/>
                                        </x-slot>
                                    </x-adminlte-input>
                                @else
                                    @include('components.text')
                                    @if(isset($value['hint_url']))
                                            <a href="{{$value['hint_url']['url']}}" @if($value['hint_url']['blank']) target="_blank" @endif>{{$value['hint_url']['word']}}</a>
                                    @endif
                                @endif
                            ...
                            @case('selectize')
                                @feature(isset($value['feature'])?$value['feature']:'nofeature',isset($value['feature'])?true:false)
                                @include('components.selectize')
                                @endfeature
                            @break

                            @case('custome-date-start-common')
                                <x-adminlte-date-range name="{{$value['name']}}" autocomplete="off">
                                    <x-slot name="appendSlot">
