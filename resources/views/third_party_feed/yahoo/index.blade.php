...
@section('content_header')
    ...
    <h1>Yahoo 供稿</h1>
    {{ Form::open(['id' => 'search', 'route' => 'yahoo_articles.list', 'method' => 'get', 'style' => 'border:1px solid #ccc; padding: 5px;']) }}
    ...
    @foreach ($datas as $k => $v)
        <tr>
            ...
            @if($cond['search_release_date'] == 'yes')
                <td class="align-middle">{{$v->toArray()['rss_release_date']}}</td>
            @else
...
@section('js')
    <script>
        ...
        function deleteLine(){
            ...
            $.ajax({
            data: lineFormData,
            url: '{{route("yahoo_articles.delete")}}',
            type: 'POST',
            dataType: 'JSON',
            cache: false,
            processData: false,
