...
@section('content_header')
    ...
    @if(isset($_GET['select_type']) && $_GET['select_type'] == 0)
        {{ Form::open(['route' => 'app_activities.index', 'method' => 'get', 'style' => 'border:1px solid #ccc; padding: 5px;']) }}
                            ...
                            @foreach ($datas as $key => $data)
                                <tr data-id="{{$key+1}}">
                                    <td class="align-middle">
                                        {{ $key+1 }}
                                        {!! Form::input('hidden', "app_activities[".($key+1)."][id]", $data->id ) !!}
                                    </td>
                                    <td class="align-middle" style="width:25%;">
                                        {{ $data->title }}
                                    </td>
                                    <td class="align-middle">
                                        <img src="{{$data->link}}" style="width: 100px">
                                    </td>
                                    <td class="align-middle">
                                        {{ $data->released }}
                                    </td>
                                    ...
                                    @if(!isset($_GET['select_type']) || $_GET['select_type'] == 1)
                                        ...
                                        <td class="align-middle">
                                            <a class="btn btn-secondary btn-flat" href="{{ route('app_activities.show', $data->id) }}">預覽</a>
                                            @can($role_name.'-edit')
...
<script>
    ...
    $(document).ready(function() {
       ...
       $('input[name="search_end"]').daterangepicker({
        ...
        $("#review_image_click").click(function(){
            if($("#link").val()){
                $("#review_image").attr("src",$("#link").val());
        ...
        $('.createBtn').on('click', function() {
            reset_modal();
            $('input[name="released"]').prop('disabled', false);
           $('input[name="released"]').daterangepicker({
                singleDatePicker: true,
                showDropdowns: true,
        ...
        $('.editBtn').on('click', function() {
            ...
            $.get(editUrl, function (data) {
                ...
                if(data.released <= '{{date("Y-m-d H:i:s")}}'){
                    $('input[name="released"]').prop('disabled', true);
                }
    ...
    {
        let AppActivitiesFormData = new FormData($('#app_activities')[0]);

        let sortId = new Array();

        $("form#app_activities tbody tr").each(function(idx){
            sortId.push($(this).attr('data-id'));
        });

        AppActivitiesFormData.append('sortId', sortId);

        let ajaxUrl2 = '';
        ajaxUrl2 = "{{url()->current()}}/"+'save_sort';
        AppActivitiesFormData.append('_method', 'PATCH');

        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('#app_activities input[name="_token"]').val()
            }
        });

        $.ajax({
            data: AppActivitiesFormData,
            url: ajaxUrl2,
            type: 'POST',
            dataType: 'JSON',
            cache: false,
    ...
    function saveAction()
    {
        ...
        var releasedDisabled =  $('input[name="released"]').prop('disabled');
        console.log('Disabled',releasedDisabled);
        ...
        $.ajax({
            ...
            error: function (err) {
                ...
                $('input[name="released"]').prop('disabled', releasedDisabled);
                console.log(err);
                if (err.status == 422) {
                    printErrorMessage(err.responseJSON.errors);
                }
